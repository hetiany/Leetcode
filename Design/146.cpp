...
class LRUCache{
public:
    void touch(unordered_map<int, pair<int, list<int>:: iterator> >:: iterator itr) {
        table.erase(itr -> second.second);
        table.push_front(itr -> first);
        itr -> second.second = table.begin();
    }
    ...
    void set(int key, int value) {
        if(cache.find(key) != cache.end()) {
            touch(cache.find(key));
            cache.find(key) -> second.first = value;
        } else {
